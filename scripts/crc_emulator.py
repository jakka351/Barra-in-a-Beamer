...
def crc_send(bus):
    ...
    """
    msg = can.Message(arbitration_id=0x0F3, data=[1, counterByte, 3, 4, 5, 6, 7, 0], is_extended_id = False)
    ...
    msg = can.Message(arbitration_id=0x0F3, data=[8, counterByte, 10, 11, 12, 13, 14, 0], is_extended_id = False)
    messages = build_crc_msgs(msg)
...
def build_crc_msgs(msg):
    ...
    for counter in range(16):
        if counterByte != 0x1E:
            counterByte = counter + 1
        elif counterByte == 0x1E:
            counterByte == 0x10
        checksum = compute_xbr_checksum(msg, counter)
        msg.data[0] = counter + (checksum << 4)
        msg.data[1] = counterByte
        messages.append(
